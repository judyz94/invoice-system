    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <select class="custom-select @error('document_type') is-invalid @enderror" id="document_type" name="document_type" required>
                ...
                <option value="CC" {{ $seller->document_type == 'CC' ? 'selected' : ''  }}>{{ __('CC') }}</option>
                <option value="TI" {{ $seller->document_type == 'TI' ? 'selected' : ''  }}>{{ __('TI') }}</option>
                <option value="CE" {{ $seller->document_type == 'CE' ? 'selected' : '' }}>{{ __('CE') }}</option>
                <option value="RC" {{ $seller->document_type == 'RC' ? 'selected' : '' }}>{{ __('RC') }}</option>
                <option value="NIT" {{ $seller->document_type == 'NIT' ? 'selected' : '' }}>{{ __('NIT') }}</option>
                <option value="RUT" {{ $seller->document_type == 'RUT' ? 'selected' : '' }}>{{ __('RUT') }}</option>
            </select>
            @error('document_type')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
